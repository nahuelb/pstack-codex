...
Model defaults live in the [bundled role registry](../../skills/setup-pstack/references/model-defaults.json). Fresh registry entries refer back to those defaults. The registry can override every single role and panel lane. Use `skill-default` to restore a bundled choice. Explicit requests provide `model` and `reasoning_effort`; a Fast request adds `"service_tier":"priority"`. An omitted tier leaves inheritance unconstrained. Setup receipts describe what the installer validated. Runtime dispatch checks supported configuration and live inheritance separately, following the [model policy](../../skills/setup-pstack/references/model-profile.md). Missing arguments or abbreviated model lists do not by themselves prove unavailability. A partial update changes only the roles you name.
