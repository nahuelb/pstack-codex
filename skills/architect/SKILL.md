...
Resolve `architect runners` through `../setup-pstack/references/model-profile.md`. The default lanes are `anthropic/claude-fable-5-1` at `high`, `gpt-6-astra` at `high`, `xai/grok-4.6` at `xhigh`, and `anthropic/claude-opus-5` at `xhigh`.
