...
- **Take a census before the next fix.** Count the imbalance per actor. Record both ownership and magnitude across repeated runs. Write the census as a rerunnable script per [Build the Lever](../principle-build-the-lever/SKILL.md).
- **Read the skew.** If the same few actors hold most of the imbalance on every run, investigate what assigns them that role. Confirm the causal link before changing assignment. That assignment is the next "why" per [Fix Root Causes](../principle-fix-root-causes/SKILL.md).
- **Remove the asymmetry instead of compensating for it**, per the [Laziness Protocol](../principle-laziness-protocol/SKILL.md). If assignment causes the defect, consider rotating, randomizing, or moving the role while preserving ordering and ownership requirements. A return path, a shared pool, a batched hand-off, or a periodic rebalance leaves the assignment in place and adds work on every run.

**Stop:**
- Do not start the next fix before the premise is written down and the census exists.
- If the census is even across actors, it does not support an assignment-skew explanation. It does not disprove every premise. Test another hypothesis and keep the census as evidence.
