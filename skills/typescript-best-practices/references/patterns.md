...
Before writing a property-by-property guard for external data, look for the repository's runtime schema library and existing schemas. Let one schema own validation and derive the TypeScript type from it. Do not maintain a schema, duplicate interface, and guard that can drift apart.
...
Use `safeParse` when failure is an expected branch. Use the equivalent inference helper with another existing schema library. Do not add a new dependency for one guard.
