...
3. Pick the runners. Resolve `arena runners` through `../setup-pstack/references/model-profile.md` and use its exact resolved lanes. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
