...
Resolve `why synthesizer` through `../setup-pstack/references/model-profile.md`, default `anthropic/claude-fable-5` with `max` reasoning. Give it read-only access to cited evidence only. If no agent is available, synthesize sequentially in the parent and label that fallback.
