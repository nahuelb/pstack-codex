...
1. Call `spawn_agent` with `agent_type: "pstack-comment-sicko"` when that profile is installed. Otherwise call `spawn_agent` with `agent_type: "default"` and seed it with `references/comment-sicko-prompt.md`. Pass the scope and require read-only analysis. Call `wait_agent` because step 2 requires the report. Never use `create_thread` or another separate-task API for this internal review. If `spawn_agent` is unavailable, report the review capability as blocked rather than self-certifying.
