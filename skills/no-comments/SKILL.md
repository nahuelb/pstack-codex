...
1. Start the `pstack-comment-sicko` custom agent with the review scope and require read-only analysis returned in its final response. The main agent saves the report; the reviewer writes no files. Follow `subagent-lifecycle`. Never use `create_thread` or another separate-task API for this internal review. If no subagent can run, report the review capability as blocked rather than self-certifying.
