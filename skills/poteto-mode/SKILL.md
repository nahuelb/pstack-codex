...
**Use the exact `pstack-poteto-agent` custom agent for the Poteto persona, subject to the compatible fallback in `subagent-lifecycle`.** Resolve each playbook delegate's model lane and configuration path through `../setup-pstack/references/model-profile.md`. Preserve its requested model, effort, and tier with explicit user overrides. Feature and refactoring use `feature, refactoring`. Bug fixes, performance work, and hillclimbs use their matching role. Use `judgment and prose` for judgment or prose work. Use `hardest tasks` for ambiguous cross-cutting design, subtle concurrency, or difficult algorithms. Routed workflow skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`) own their named registry roles.
