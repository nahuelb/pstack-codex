...
`$poteto-mode` must lead the user request. The activation hook ignores the app-added `## Referenced chats with Codex:` preamble and one leading slash command such as `/goal`. This explicit invocation applies the complete skill on the activation turn. A trusted bundled hook records only session-scoped activation metadata for later turns. Casual mentions, quoted examples, and the words “poteto mode” do not activate it.
...
- **Attack the Premise** (**principle-attack-the-premise**). Repeated fixes share a failed premise. Measure actor imbalance and test the shared assumption before another fix.
...
- **Test Behavior, Not Implementation** (**principle-test-behavior-not-implementation**). Writing or reviewing tests. Check observable outcomes and relevant defects while preserving required contracts.
...
**Use the exact `pstack-poteto-agent` custom agent for the Poteto persona, subject to the compatible fallback in `references/codex-subagent-lifecycle.md`.** Resolve each playbook delegate's model lane and configuration path through `../setup-pstack/references/model-profile.md`. Preserve its requested model, effort, and tier with explicit user overrides. Feature and refactoring use `feature, refactoring`. Bug fixes, performance work, and hillclimbs use their matching role. Use `judgment and prose` for judgment or prose work. Use `hardest tasks` for ambiguous cross-cutting design, subtle concurrency, or difficult algorithms. Routed workflow skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`) own their named registry roles.
...
- **Label evidence with the claim.** State whether it was measured, inferred, or remains a hypothesis. Run available checks before asking the user to verify.
