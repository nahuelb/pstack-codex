...
/** Legacy GitHub review input, not a Codex runtime dependency. */
export const LEGACY_CURSOR_REVIEW_INPUT = {
  authorLogin: "cursor",
  marker: "CURSOR_AUTOMATION_ID",
} as const;
...
function isBugbot(comment: T.ReviewComment | null): boolean {
  ...
  return (
    ...
    (author === LEGACY_CURSOR_REVIEW_INPUT.authorLogin &&
      [
        "bugbot",
        LEGACY_CURSOR_REVIEW_INPUT.marker.toLowerCase(),
...
function passKey(comment: T.ReviewComment | null): string | null {
  ...
  for (const pattern of [
    /RUN_ID:\s*([a-zA-Z0-9_.:-]+)/,
    new RegExp(`${LEGACY_CURSOR_REVIEW_INPUT.marker}:\\s*([a-zA-Z0-9_.:-]+)`),
  ]) {
