...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the `bug-fix` lane from the pstack model registry, default `anthropic/claude-fable-5-1` with `xhigh` reasoning, with a specific scope; review the diff.
