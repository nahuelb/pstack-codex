...
3. Explore with the `pstack-poteto-agent` custom agent. Follow the runtime contract. Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
**Verification.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked (the **prove-it-works** principle skill). That sentence is the verification rule. Every verification block opens with it. The live block is mandatory. Ten lanes using `swarm workers` at the PR head drive the real surface through its control skill. One regression lane runs the same load-bearing scenario on trunk and head. If trunk lacks the feature, record that fact and gate the new behavior plus the end state the user expects. The perf gate measures both trunk and head. When trunk lacks the feature, isolate the new work and set absolute budgets for that work and the expected end state. Do not compare unlike scenarios as a ratio. A PR that changes an interaction remains review-gated.
...
- [ ] Open the PR ready, never draft, with `gh pr create --base <base-branch>` or a PR API call with `draft: false`. A stack child targets its parent branch.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes using `swarm workers` at the PR head, per the boot recipe.

- [ ] Lane 1. Regression lane against trunk. Run <the same load-bearing scenario> at trunk and head. If trunk lacks the feature, record that and gate <the new behavior plus the end state the user expects>. Save `<slug>.png`. Pass when <predicate>.
...
- [ ] Metric. <What is measured at trunk and head. If trunk lacks the feature, also name the new work and expected end state.>
- [ ] Probe. <The command or procedure, run at trunk and at the head, interleaved. Both sides produce the metric.>
- [ ] Baseline. Record the trunk <value> first.
- [ ] Rule. <Head against trunk, with the number that fails. If scenarios differ, set absolute budgets for new work and the expected end state.>
...
- [ ] Main agent's clean verdict at the exact head SHA.
- [ ] Bugbot triage done.
- [ ] Rebased onto current trunk after the verdict, patch-id unchanged.
- [ ] <The owner squash-merges its own PR, or the main agent appends it to the base-branch stack and the operator lands it bottom-up.>
