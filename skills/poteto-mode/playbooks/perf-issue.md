...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the `perf-issue` lane from the pstack model registry, default `anthropic/claude-fable-5-1` with `high` reasoning; review the diff. Capture a post-fix trace.
