...
New receipts store a hashed input snapshot containing canonical cwd, the relevant contract, sorted file inventory, executable bits, content hashes, and base64 bytes. The input inventory reconstructs `sourceHash`; snapshot and artifact hashes bind the captured evidence to the receipt. Source changes during preflight prevent proof execution. Changes during execution make the receipt stale. Attached reviewed artifacts are copied into the run directory, so later edits to their original paths cannot replace captured evidence. Original-file changes still invalidate freshness, preserving the existing integrity gate.
