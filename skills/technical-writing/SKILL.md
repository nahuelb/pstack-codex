...
Don't invent jargon. Use the words a developer would say out loud: "move", "delete", "a budget that only decreases", not "evacuate", "ratchet", or "endgame". A named pattern is fine when the doc says what it means the first time. Propose new offenders and replacements in your reply. Do not edit `unslop` unless the active request authorizes that change.
...
- PR descriptions and commit messages are writing too. Every layer except Diátaxis applies to them. Keep the PR body brief and link detailed logs, commit lists, and metric tables. Retain evidence needed to assess the change.
